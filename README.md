...
- ClawNode 会通过 mDNS 发现网关，并**优先使用 mDNS 名称**（如 `miniorange-xxx.local`）连接与持久化，不再依赖固定 IP。
- 网关换 Wi-Fi / DHCP 分配新 IP 后，设备仍能通过 mDNS 自动重新连上。
- 若本地仍保存了旧的固定 IP，升级后会自动迁移为 `ws://auto` 并通过 mDNS 重连。
