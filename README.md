...
- 推荐留空或填 `ws://auto`，或使用「发现网关并配对」。
- ClawNode 会通过 mDNS 发现网关，并**优先使用服务器广播的 mDNS 名称**（如 `miniorange-changpengchengdeMacBook-Pro.local`）来保存/连接，而不是只存固定 IP。
- 这样当 Mac 换 Wi-Fi / DHCP 分配新 IP 后，设备仍能通过 mDNS 名称解析到当前 IP 重新连上。
- 如果手动填写固定 IP，则会按固定 IP 连接，IP 变化后需手动更新或重新配对。
